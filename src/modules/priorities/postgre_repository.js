...
const prioritiesColumn = require('./column')

class PrioritiesRepository {
  constructor() {
    this.table = 'ticket_priorities'
    this.column = prioritiesColumn
  }
  ...
  async findByLevel(level) {
    return await fetchByParam(this.table, { level }, Object.values(this.column))
  }
  ...
  async checkNameExists(name, excludeId = null) {
    ...
    return await checkSameValueinDb(this.table, where, 'name', 'Nama prioritas sudah ada')
  }

  async checkLevelExists(level, excludeId = null) {
    const where = { level }
    if (excludeId) {
      where.id = { '!=': excludeId }
    }
    return await checkSameValueinDb(this.table, where, 'level', 'Level prioritas sudah ada')
  }
}

module.exports = new PrioritiesRepository()
